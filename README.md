...
- **Backspace（消去）キー**
  - タップ：1文字消す
  - 長押し：Control キーとして動作（他のキーと組み合わせて使用）
...
└─────┴─────┴─────┴─────┴─────┴───────┘
      ┌──────────┬──────────┬───────┐
      │ Backsp※5 │ Enter※6  │  Del  │
      └──────────┴──────────┴───────┘

※1 長押しで Shift キー
※2 長押しで設定レイヤー
※3 長押しで矢印キーレイヤー
※4 長押しでスクロールレイヤー（トラックボール用）
※5 長押しで Control キー
※6 長押しでファンクションキーレイヤー
```
...
**Enter キーを長押ししている間**、このレイヤーが有効になります。
...
説明:
- Esc: エスケープキー（キャンセル）
- 前タブ: Ctrl+Shift+Tab（ブラウザのタブを左に移動）
- 次タブ: Ctrl+Tab（ブラウザのタブを右に移動）
- 選択拡張: Cmd+Shift+矢印（テキスト選択を単語単位で拡張）
```

使い方の例：
- 文章を編集するときに矢印キーでカーソル移動
- Home/End で行の先頭・末尾にジャンプ
- ブラウザでタブ切り替え
